...
func TestService_Convert_OnlyCustomLinks(t *testing.T) {
	...
	if len(resp.Proxies) == 0 {
		t.Fatalf("expected proxies from custom links")
	}
}
...
func TestService_Convert_MergeCustomLinks(t *testing.T) {
	...
	if len(resp.Proxies) < 1 {
		t.Fatalf("expected merged proxies from both urls and extra links")
	}
}
...
func TestService_Convert_CustomDirectNodeLinks(t *testing.T) {
	...
	if len(resp.Proxies) < 1 {
		t.Fatalf("expected proxies parsed from direct node links")
	}
}
